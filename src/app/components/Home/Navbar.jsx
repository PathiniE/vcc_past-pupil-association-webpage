...
export function Navbar() {
  ...
  return (
    <header className="relative border-b">
      ...
      <div 
        ref={exploreMenuRef}
        className={`absolute left-0 top-full z-50 w-full border-b bg-white shadow-lg lg:block transition-all duration-300 ease-in-out origin-top ${
          isExploreOpen ? "opacity-100 transform scale-y-100" : "opacity-0 transform scale-y-0 h-0 overflow-hidden"
        }`}
      >
        <div className="mx-auto max-w-7xl px-4 py-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      ...
      {/* Mobile Menu - With Improved Smooth Animation */}
      <div
        ref={mobileMenuRef}
        className={`absolute left-0 top-full z-50 w-full border-b bg-white lg:hidden transition-all duration-300 ease-in-out transform ${
        ...
        <div className="flex flex-col space-y-4 p-4">
          ...
          <div className="space-y-4">
            ...
            >
              <span>Explore</span>
              <ChevronDown
                className={`h-5 w-5 transition-transform duration-300 ${isMobileExploreOpen ? "-rotate-180" : ""}`}
              />
            </button>
            <div 
              className={`transition-all duration-300 ease-in-out ${
                isMobileExploreOpen ? "max-h-[300px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"
              }`}
            >
              <div className="space-y-4 pl-4 pt-2">
                {exploreItems.map((item) => (
